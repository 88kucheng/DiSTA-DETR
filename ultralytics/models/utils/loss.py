...
class DETRLoss(nn.Module):
    ...
    #     loss[name_bbox] = self.loss_gain['bbox'] * F.l1_loss(pred_bboxes, gt_bboxes, reduction='sum') / len(gt_bboxes)
    #
    #     # Dynamic fusion of NWD and MPDIoU
    #     if self.use_dynamic_nwd_mpdiou:
    #         nwd_loss = 1.0 - wasserstein_loss(pred_bboxes, gt_bboxes)
    #         mpdiou_loss = 1.0 - bbox_mpdiou(pred_bboxes, gt_bboxes, xywh=True, mpdiou_hw=2)
    #
    #         # Compute area = w * h
    #         areas = gt_bboxes[..., 2] * gt_bboxes[..., 3]  # [N]
    #         # weights = torch.sigmoid((self.dynamic_nwd_thresh - areas) / 40.0)  # small object -> weight→1
    #         weights = torch.sigmoid((self.dynamic_nwd_thresh - areas) / 100.0)
    #
    #         combined = weights * nwd_loss + (1 - weights) * mpdiou_loss
    #         loss[name_giou] = self.loss_gain['giou'] * combined.mean()
    #     else:
    #         loss[name_giou] = self.loss_gain['giou'] * (
    #                     1.0 - bbox_mpdiou(pred_bboxes, gt_bboxes, xywh=True, mpdiou_hw=2)).mean()
    #
    #     return {k: v.squeeze() for k, v in loss.items()}
    #
    # # ------------------------------------------------------------------------------------
